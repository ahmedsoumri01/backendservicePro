...
const setupSocket = (server) => {
  ...
  io.use(async (socket, next) => {
    try {
      ...
      if (user.status !== "active") {
        ...
        return next(
          new Error("Authentication error: User account is not active")
        );
      }
      ...
      socket.user = user;
      console.log(
        `Socket authenticated for user: ${user._id} (${user.firstName} ${user.lastName})`
      );
      next();
    } catch (error) {
  ...
  io.on("connection", (socket) => {
    ...
    console.log(
      `User connected: ${userId} (${socket.user.firstName} ${socket.user.lastName})`
    );
    ...
    socket.on("send_message", async (messageData) => {
      ...
      console.log(
        `Message sent in conversation ${messageData.conversation} by user ${userId}`
      );
